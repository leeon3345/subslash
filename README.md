...
비밀번호는 되돌릴 수 없게 저장하므로 알려드릴 수 없습니다. 로그인 화면의 "비밀번호를
잊으셨나요?"에서 가입한 이메일을 적으면, 그 주소로 새 비밀번호를 정하는 링크가 갑니다
(`/forgot-password` → 메일 → `/reset-password`).

- **링크는 60분 동안, 한 번만 쓸 수 있습니다.** 링크를 DB에 적어두지 않고, 대신 링크에
  지금 비밀번호 해시의 지문을 담습니다. 비밀번호가 바뀌면 해시가 달라져 쓴 링크와 그
  전에 보낸 링크가 함께 무효가 됩니다. 주소가 바뀌어도 무효입니다.
- **링크를 여는 것만으로는 아무것도 바뀌지 않습니다.** 새 비밀번호를 적어 제출해야
  바뀝니다(가입 확인과 같은 이유 — 메일 검사기).
- **바꾸면 모든 기기의 로그인이 끊기고**, 바꾼 브라우저만 새 세션으로 로그인됩니다.
  주소로 간 링크를 열었으므로 이메일도 '확인됨'이 됩니다.
- 가입 확인 링크로는 비밀번호를 바꿀 수 없고, 그 반대도 마찬가지입니다(링크의 용도 `act`가 다름).
- 메일은 **가입 확인 메일과 주소별 한도를 함께 씁니다**(1분 1통, 24시간 5통).
- 가입되지 않은 주소면 그렇다고 알려줍니다. 가입 폼이 이미 "이미 가입된 이메일"을
  알려주므로 여기서 숨겨도 가입 여부는 감춰지지 않고, 주소를 잘못 적은 사람만 오지 않을
  메일을 기다리게 됩니다.

### 메일을 보내는 엔드포인트의 IP 제한 (Vercel WAF)

가입, 확인 메일 재발송, 비밀번호 재설정 메일, 알림 신청은 모르는 사람에게 메일을 보낼 수 있는 입구입니다.
...
| 조건 | `POST` 그리고 경로가 `/api/auth/signup`, `/api/auth/verification-email`, `/api/auth/password-reset`, `/api/notify/subscribe` 중 하나 |
