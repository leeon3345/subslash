...
type PendingConfirm = "rotate" | "disconnect";
...
export function GmailAutoImportSetup() {
  ...
  const runConfirm = () => {
    ...
    if (kind === "rotate") void issueToken();
    ...
    <section className="space-y-3 rounded-2xl border p-4">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground">
          내 Google 계정의 Apps Script가 2주마다 새 결제 메일을 찾아 SubSlash로 보냅니다. SubSlash는
          메일에서 구독 후보(서비스·금액·결제일·보낸 사람)만 남기고 메일 제목·본문은 저장하지
          않습니다. 알려진 서비스의 최근 결제는 SubSlash를 열 때 바로 등록되고, 확실하지 않은 것은
          확인을 받습니다.
        </p>
      </div>
          ...
          {link.linked && (
            <div className="rounded-xl bg-muted/50 p-3 text-xs leading-relaxed">
              <p className="font-semibold">
                연결됨 · {formatDateTime(link.createdAt)}에 스크립트 발급
              </p>
              <p className="text-muted-foreground">
                {link.lastIngestAt
          ...
          {token ? (
            <div className="space-y-3">
              <ol className="list-decimal space-y-2 pl-5">
                ...
                <li>
                  왼쪽의 톱니바퀴(프로젝트 설정)에서 <code>appsscript.json</code> 매니페스트 파일을
                  편집기에 표시하도록 켭니다.
                </li>
                <li>
                  <code>appsscript.json</code> 내용을 아래 매니페스트로, <code>Code.gs</code> 내용을
                  아래 스크립트로 바꾸고 저장합니다.
                </li>
                <li>
                  위쪽 함수 목록에서 <code>setup</code>을 골라 실행하고 권한을 허용합니다. 직접 만든
                  스크립트라 Google이 확인하지 않은 앱이라는 경고가 나오며, &lsquo;고급&rsquo;에서
                  계속할 수 있습니다.
                </li>
                <li>
